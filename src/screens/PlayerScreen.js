...
class PlayerScreen extends BaseScreen {
    ...
    constructor(props) {
        ...
        if (this.player.track !== null && this.player.track.id === this.item.id) this.needsPrapare = false;
        this.player.setTrack(this.item,this.type,this.typeId);
        setTimeout(() => {
            if (this.needsPrapare && (this.item.canPlay === 1 || TRACK_PLAY_ACCESS === 1 || this.type === 'radio')) this.player.prepare();
        }, 300);
        this.state = {
            ...this.state,
    ...
    componentDidMount() {
        if ((!this.player.playing && !this.player.isPaused) || this.needsPrapare) {
            ...
            <ImageBackground blurRadius={2} source={{uri: this.item.art_lg}} style={{flex:1,width:'100%',height:'100%'}}>
                <View style={{backgroundColor: 'rgba(0,0,0,0.8)', flex : 1, paddingTop:15,flexDirection:'row',alignContent:'center'}}>
                    ...
                    <View style={{alignSelf:'center', alignContent: 'center', padding:20,flexDirection:'column',flex:1}}>
                        ...
                        {this.type !== 'radio' ? (<View style={{padding:10,marginTop:40,marginBottom: 10}}>
                            <TrackProgressComponent key={this.player.track.id} track={this.player.track} player={this.player}/>
                        </View>) : null}

                        <View style={{alignContent:'center', flexDirection:'row',alignSelf: 'center', marginTop: 25}}>
                            <TouchableOpacity onPress={() => {
